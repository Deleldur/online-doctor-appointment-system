...
public class HelloPatientApplication {

	private static final Logger log = LoggerFactory.getLogger(HelloPatientApplication.class);

	public static void main(String[] args) {
		SpringApplication.run(HelloPatientApplication.class, args);
...
//			repository.deleteAll();
			...
			// FIND USER BY ID
			try {
				Patient patient = repository.findById(user1.getId()).get();
				log.info("user: {}", patient);

				List<Patient> patients = mtemplate.findAllPatients_aggregation();
				log.info("user: {}", patients);
